    ...
    <div className="bg-zinc-200/50 dark:bg-white/5 rounded-lg border border-zinc-200 dark:border-gray-800 p-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {roles.map((role) => (
          <div
            key={role}
            className="rounded-lg border border-zinc-200 dark:border-gray-800 bg-zinc-100/90 dark:bg-black/30 p-4 space-y-3"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-medium text-zinc-900 dark:text-white">{ROLE_LABELS[role] ?? role}</div>
                  <div className="text-xs text-zinc-500 dark:text-gray-500">{role}</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-semibold text-zinc-900 dark:text-white">{counts[role] ?? 0}</div>
                <div className="text-xs text-zinc-500 dark:text-gray-500">users</div>
              </div>
            </div>
            {ROLE_DESCRIPTIONS[role] && (
              <p className="text-xs text-zinc-500 dark:text-gray-400">{ROLE_DESCRIPTIONS[role]}</p>
            )}
