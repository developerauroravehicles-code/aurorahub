...
const EVENT_LABELS: Record<string, { label: string; icon: typeof LogIn; color: string }> = {
  ...
  logout: { label: 'Logout', icon: LogOut, color: 'text-zinc-500 dark:text-gray-400' },
      ...
      <div className="bg-zinc-200/50 dark:bg-white/5 rounded-lg border border-zinc-200 dark:border-gray-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-zinc-200 dark:border-gray-800 text-zinc-500 dark:text-gray-400">
            ...
            <tbody>
              {logs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-zinc-500 dark:text-gray-500">
                    No records yet.
                  </td>
                </tr>
              ) : (
                logs.map((log) => {
                  const info = EVENT_LABELS[log.event_type] ?? { label: log.event_type, icon: LogIn, color: 'text-zinc-500 dark:text-gray-400' }
                  const Icon = info.icon
                  return (
                    <tr key={log.id} className="border-b border-zinc-200 dark:border-gray-800/50 hover:bg-zinc-200/50 dark:bg-white/5">
                      <td className="px-4 py-3 text-zinc-600 dark:text-gray-300">{formatDate(log.created_at)}</td>
                      ...
                      <td className="px-4 py-3 text-zinc-600 dark:text-gray-300">{log.email ?? '—'}</td>
                      <td className="px-4 py-3 font-mono text-xs text-zinc-500 dark:text-gray-400">{log.ip_address ?? '—'}</td>
                      <td className="max-w-[200px] truncate px-4 py-3 text-zinc-500 dark:text-gray-400" title={log.user_agent ?? ''}>
                        {log.user_agent ?? '—'}
                      </td>
                    </tr>
                  )
