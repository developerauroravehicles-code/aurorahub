  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-white mt-4">Identity – Genel Bakış</h2>
        ...
        {cards.map((card) => {
          ...
          return (
            <Link
              key={card.href}
              href={card.href}
              className="group flex items-center gap-4 rounded-lg border border-zinc-200 dark:border-gray-800 bg-zinc-200/50 dark:bg-white/5 p-5 transition-colors hover:bg-zinc-200 dark:bg-white/10 hover:border-zinc-300 dark:border-gray-700"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {card.subtitle && <div className="text-xs text-zinc-500 dark:text-gray-500 mt-0.5">{card.subtitle}</div>}
              </div>
            </Link>
          )
        })}
