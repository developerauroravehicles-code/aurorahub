...
export function DatabaseManagementContent({ dealers, profiles, errors }: { dealers: any[], profiles: any[], errors: any }) {
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-zinc-900 dark:text-white mb-2">Database Contents (Debug)</h3>
        <p className="text-sm text-zinc-500 dark:text-gray-400 mb-4">View all data in the database for debugging purposes</p>
      </div>

      {/* Dealers List */}
      <div>
        <h4 className="font-semibold text-zinc-500 dark:text-gray-400 mb-2">Dealers ({dealers.length})</h4>
          ...
          <table className="min-w-full divide-y divide-zinc-200 dark:divide-gray-800 text-sm text-zinc-600 dark:text-gray-300">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-gray-800">
              {dealers.map((dealer: any) => (
                <tr key={dealer.id}>
                  <td className="px-4 py-2 font-medium text-zinc-900 dark:text-white">{dealer.name}</td>
                  <td className="px-4 py-2 text-[#C27E00]">{dealer.code}</td>
                  <td className="px-4 py-2 text-zinc-500 dark:text-gray-500 text-xs font-mono">{dealer.id}</td>
                </tr>
              ))}
              {dealers.length === 0 && (
                <tr><td colSpan={3} className="px-4 py-4 text-center text-zinc-500 dark:text-gray-500">No dealers found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* Profiles List */}
      <div>
        <h4 className="font-semibold text-zinc-500 dark:text-gray-400 mb-2">User Profiles ({profiles.length})</h4>
          ...
          <table className="min-w-full divide-y divide-zinc-200 dark:divide-gray-800 text-sm text-zinc-600 dark:text-gray-300">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-gray-800">
              {profiles.map((profile: any) => (
                <tr key={profile.id}>
                  <td className="px-4 py-2 font-medium text-zinc-900 dark:text-white">{profile.full_name}</td>
