  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-white">{profile.full_name ?? personnel?.full_name ?? '—'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase">Phone</p>
              <p className="text-white">{personnel?.phone ?? profile.phone ?? '—'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-white">{personnel?.position ?? '—'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase">Status</p>
              <p className="text-white capitalize">{personnel?.status ?? '—'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase">Start Date</p>
              <p className="text-white">{personnel?.start_date ? new Date(personnel.start_date).toLocaleDateString() : '—'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase">Province</p>
              <p className="text-white capitalize">{personnel?.province?.replace('_', ' ') ?? '—'}</p>
            </div>
            <div>
        ...
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          ...
          {showLeaveForm && (
            <form
              ...
              className="mb-6 p-4 rounded-lg bg-black/30 border border-gray-700 space-y-4"
            >
              <h3 className="text-md font-medium text-white">New Leave Request</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="leave_type" required className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
                    {Object.entries(LEAVE_TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Start Date</label>
                  <input name="start_date" type="date" required className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">End Date</label>
                  <input name="end_date" type="date" required className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Notes</label>
                <textarea name="notes" rows={2} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" placeholder="Optional" />
              </div>
              <div className="flex gap-2">
                <button type="submit" className="rounded bg-[#C27E00] px-4 py-2 text-sm text-black hover:bg-amber-500">Submit</button>
                <button type="button" onClick={() => { setShowLeaveForm(false); setLeaveError(null) }} className="rounded bg-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-600">Cancel</button>
              </div>
            </form>
          )}
              ...
              <table className="min-w-full divide-y divide-gray-800 text-sm">
                <thead>
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-gray-400">Notes</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {leaveRequests.map((l) => (
                    <tr key={l.id}>
                      <td className="px-4 py-2 text-gray-300">{LEAVE_TYPE_LABELS[l.leave_type] ?? l.leave_type}</td>
                      <td className="px-4 py-2 text-gray-400">{new Date(l.start_date).toLocaleDateString()} – {new Date(l.end_date).toLocaleDateString()}</td>
                      <td><span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[l.status] ?? 'bg-gray-700 text-gray-400'}`}>{l.status}</span></td>
                      <td className="px-4 py-2 text-gray-400 max-w-[200px] truncate">{l.notes ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div className="rounded-lg border border-amber-500/30 bg-amber-900/10 p-4">
                    ...
                    <p className="text-2xl font-semibold text-white">${pendingTotal.toLocaleString()}</p>
                    <p className="text-gray-500 text-sm mt-1">{pendingPayments.length} pending payment(s)</p>
                  </div>
                  <div className="rounded-lg border border-gray-700 bg-black/30 p-4">
                    ...
                    <p className="text-2xl font-semibold text-white">${paidTotal.toLocaleString()}</p>
                    <p className="text-gray-500 text-sm mt-1">{paidPayments.length} paid payment(s)</p>
                  </div>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-800 text-sm">
                      <thead>
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-gray-400">Status</th>
                          <th className="px-4 py-2 text-left text-gray-400">Paid At</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-800">
                        {payments.map((p) => (
                          <tr key={p.id}>
                            <td className="px-4 py-2 text-gray-400">{p.period_start && p.period_end ? `${new Date(p.period_start).toLocaleDateString()} – ${new Date(p.period_end).toLocaleDateString()}` : '—'}</td>
                            <td className="px-4 py-2 text-gray-300">{PAYMENT_TYPES[p.payment_type ?? ''] ?? p.payment_type ?? '—'}</td>
                            <td className="px-4 py-2 text-right text-white">${Number(p.amount).toLocaleString()}</td>
                            ...
                            <td className="px-4 py-2 text-gray-400">{p.paid_at ? new Date(p.paid_at).toLocaleDateString() : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {activeTab === 'it_request' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          <h2 className="text-lg font-semibold text-white mb-4">IT Support Request</h2>
          <p className="text-gray-400 text-sm mb-4">Submit a support ticket for IT. You will be notified when your request is processed.</p>
          ...
          {!showItForm ? (
            ...
            >
              ...
              <div>
                ...
                <input name="title" type="text" required placeholder="Brief description" className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Category</label>
                <select name="category" className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
              ...
              <div>
                ...
                <select name="priority" className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
              ...
              <div>
                ...
                <textarea name="description" rows={4} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" placeholder="Details of your request..." />
              </div>
              <div className="flex gap-2">
                <button type="submit" className="rounded bg-[#C27E00] px-4 py-2 text-sm text-black hover:bg-amber-500">Submit</button>
                <button type="button" onClick={() => { setShowItForm(false); setItError(null) }} className="rounded bg-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-600">Cancel</button>
              </div>
            </form>
          )}
      ...
      {activeTab === 'documents' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          ...
          {complianceDocuments.length === 0 ? (
            <p className="text-gray-500">No compliance documents.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-800 text-sm">
                <thead>
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-gray-400">Status</th>
                    <th className="px-4 py-2 text-left text-gray-400">Link</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {complianceDocuments.map((d) => {
                    ...
                    return (
                      <tr key={d.id} className={expired ? 'bg-red-500/5' : ''}>
                        <td className="px-4 py-2 text-gray-300">{DOC_TYPES[d.document_type ?? ''] ?? d.document_type ?? '—'}</td>
                        <td className="px-4 py-2 text-gray-400">{d.title ?? '—'}</td>
                        <td className="px-4 py-2"><span className={expired ? 'text-red-400' : 'text-gray-400'}>{d.expiry_date ? new Date(d.expiry_date).toLocaleDateString() : '—'}</span></td>
                        <td>{d.verified_at ? <span className="px-2 py-0.5 rounded text-xs bg-green-500/20 text-green-400">Verified</span> : <span className="px-2 py-0.5 rounded text-xs bg-gray-700 text-gray-400">Pending</span>}</td>
          ...
          <h3 className="text-md font-medium text-white mt-6 mb-2">Compliance Checklists</h3>
            ...
            <ul className="space-y-2">
              {complianceChecklists.map((c) => (
                <li key={c.id} className="flex items-center gap-2">
                  ...
                  <span className={c.completed ? 'text-gray-500 line-through' : 'text-white'}>{c.item_name}</span>
                  {c.completed_at && <span className="text-gray-500 text-xs">({new Date(c.completed_at).toLocaleDateString()})</span>}
                </li>
              ))}
            </ul>
          )}
                  ...
                  {certifications.map((c) => {
                    ...
                    return (
                      <tr key={c.id} className={expired ? 'bg-red-500/5' : ''}>
                        <td className="px-4 py-2 text-gray-300">{CERT_TYPES[c.certification_type] ?? c.certification_type}</td>
                        <td className="px-4 py-2 text-gray-400">{c.name ?? '—'}</td>
                        <td className="px-4 py-2 text-gray-400">{new Date(c.issue_date).toLocaleDateString()}</td>
                        <td><span className={expired ? 'text-red-400' : 'text-gray-400'}>{c.expiry_date ? new Date(c.expiry_date).toLocaleDateString() : '—'}</span></td>
                        <td><span className={`px-2 py-0.5 rounded text-xs ${c.status === 'approved' ? 'bg-green-500/20 text-green-400' : 'bg-gray-700 text-gray-400'}`}>{c.status ?? '—'}</span></td>
                      </tr>
                    )
                  })}
                </tbody>
      ...
      {activeTab === 'equipment' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Assigned Equipment</h2>
          {equipment.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-800 text-sm">
                ...
                <tbody className="divide-y divide-gray-800">
                  {equipment.map((e) => (
                    <tr key={e.id}>
                      <td className="px-4 py-2 text-white">{(e.equipment_types as { name?: string } | null)?.name ?? e.item_name ?? '—'}</td>
                      <td className="px-4 py-2 text-gray-400">{e.serial_number ?? '—'}</td>
                      <td className="px-4 py-2 text-gray-400">{new Date(e.assigned_at).toLocaleDateString()}</td>
      ...
      {activeTab === 'schedule' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
              ...
              {[0, 1, 2, 3, 4, 5, 6].map((day) => {
                ...
                return (
                  <div key={day} className="flex items-center gap-4">
                    <span className="w-12 text-gray-400">{DAY_LABELS[day]}</span>
                    <span className={slot?.is_available ? 'text-green-400' : 'text-gray-500'}>
          ...
          {leaveBlocks.length > 0 && (
            <>
              <h3 className="text-md font-medium text-white mt-6 mb-2">Leave Blocks</h3>
              <ul className="space-y-2">
                {leaveBlocks.map((b) => (
                  <li key={b.id} className="text-gray-400">
      ...
      {activeTab === 'feedback' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Performance Feedback</h2>
          {feedback.length === 0 ? (
            <p className="text-gray-500">No feedback yet.</p>
          ) : (
            <ul className="space-y-4">
              {feedback.map((f) => (
                <li key={f.id} className="p-4 rounded bg-black/30 border border-gray-700">
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-gray-400 text-sm">{f.feedback_type ?? 'Feedback'} {f.source && `• ${f.source}`}</span>
                  ...
                  {f.comment && <p className="text-gray-300 mt-2">{f.comment}</p>}
                </li>
              ))}
            </ul>
          )}
      ...
      {activeTab === 'onboarding' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          ...
          {onboardingTasks.length === 0 ? (
            <p className="text-gray-500">No onboarding tasks.</p>
          ) : (
            <ul className="space-y-3">
              {onboardingTasks.map((t) => (
                <li key={t.id} className="flex items-center gap-3">
                  ...
                  <span className={t.status === 'completed' ? 'text-gray-500 line-through' : 'text-white'}>{t.title}</span>
                  {t.due_date && <span className="text-gray-500 text-xs">Due: {new Date(t.due_date).toLocaleDateString()}</span>}
