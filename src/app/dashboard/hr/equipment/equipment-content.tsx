      ...
      {activeTab === 'types' && (
        <div className="bg-zinc-200/50 dark:bg-white/5 rounded-lg border border-zinc-200 dark:border-gray-800 p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-zinc-900 dark:text-white">Equipment Types</h2>
            <button
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-zinc-200 dark:divide-gray-800 text-sm">
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-gray-800">
                {types.map((t) => (
                  <tr key={t.id}>
                    <td className="px-4 py-2 text-zinc-900 dark:text-white">{t.name}</td>
                    <td className="px-4 py-2 text-zinc-500 dark:text-gray-400">{EQUIPMENT_CATEGORIES[t.category ?? ''] ?? t.category ?? '—'}</td>
                    <td className="px-4 py-2 text-right">
                      <button onClick={() => { setEditingTypeId(t.id); setShowTypeForm(true) }} className="p-1.5 text-zinc-500 dark:text-gray-400 hover:text-[#C27E00] mr-1" title="Edit">
            ...
            {types.length === 0 && !showTypeForm && (
              <p className="text-zinc-500 dark:text-gray-500 py-6 text-center">No equipment types. Add dashcam demo, tools, testing equipment, etc.</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="bg-zinc-200/50 dark:bg-white/5 rounded-lg border border-zinc-200 dark:border-gray-800 p-6">
            ...
            <div className="flex gap-2">
              {(['active', 'returned', 'all'] as const).map((f) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-sm ${assignFilter === f ? 'bg-[#C27E00] text-white' : 'bg-zinc-200 dark:bg-white/10 text-zinc-500 dark:text-gray-400 hover:text-white'}`}
                >
                  {f === 'active' && 'Active'}
                  {f === 'returned' && 'Returned'}
                  {f === 'all' && 'All'}
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-zinc-200 dark:divide-gray-800 text-sm">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-zinc-500 dark:text-gray-400">Equipment</th>
                  ...
                  <th className="px-4 py-2 text-left text-zinc-500 dark:text-gray-400">Returned</th>
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-gray-800">
                {filteredAssignments.map((a) => (
                  <tr key={a.id} className={a.returned_at ? 'opacity-70' : ''}>
                    ...
                    <td className="px-4 py-2 text-zinc-600 dark:text-gray-300">{a.equipment_types?.name ?? a.item_name ?? '—'}</td>
                    <td className="px-4 py-2 text-zinc-500 dark:text-gray-400">{a.serial_number ?? '—'}</td>
                    <td className="px-4 py-2 text-zinc-500 dark:text-gray-400">{new Date(a.assigned_at).toLocaleDateString()}</td>
                    <td className="px-4 py-2 text-zinc-500 dark:text-gray-400">{a.returned_at ? new Date(a.returned_at).toLocaleDateString() : '—'}</td>
                    <td className="px-4 py-2 text-zinc-500 dark:text-gray-400">{a.condition ?? '—'}</td>
                    <td className="px-4 py-2 text-right">
                      {!a.returned_at && (
                        <form action={async () => { await returnEquipment(a.id); router.refresh() }} className="inline mr-1">
                          <button type="submit" className="p-1.5 text-zinc-500 dark:text-gray-400 hover:text-green-400" title="Mark returned">
                            <RotateCcw className="w-4 h-4 inline" />
                          </button>
                        </form>
                      )}
                      <button onClick={() => { setEditingAssignId(a.id); setShowAssignForm(true) }} className="p-1.5 text-zinc-500 dark:text-gray-400 hover:text-[#C27E00] mr-1" title="Edit">
                        <Pencil className="w-4 h-4 inline" />
                      </button>
            ...
            {filteredAssignments.length === 0 && !showAssignForm && (
              <p className="text-zinc-500 dark:text-gray-500 py-6 text-center">
...
function TypeForm({ type, onClose, onSuccess }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 p-4 rounded bg-zinc-100/90 dark:bg-black/30 border border-zinc-300 dark:border-gray-700 space-y-3">
      <h3 className="text-zinc-900 dark:text-white font-medium">{isEdit ? 'Edit Type' : 'Add Equipment Type'}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Name</label>
          <input name="name" required defaultValue={type?.name ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm" placeholder="e.g. Dashcam Demo Unit" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Category</label>
          <select name="category" defaultValue={type?.category ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm">
            <option value="">—</option>
...
function AssignmentForm({ personnel, types, assignment, onClose, onSuccess }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 p-4 rounded bg-zinc-100/90 dark:bg-black/30 border border-zinc-300 dark:border-gray-700 space-y-3">
      <h3 className="text-zinc-900 dark:text-white font-medium">{isEdit ? 'Edit Assignment' : 'Assign Equipment'}</h3>
      {!isEdit && (
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Equipment Type</label>
          <select name="equipment_type_id" defaultValue={assignment?.equipment_type_id ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm">
            <option value="">— Or use custom name below</option>
            {types.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Item Name (if no type)</label>
          <input name="item_name" defaultValue={assignment?.item_name ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm" placeholder="Custom equipment name" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Serial Number</label>
          <input name="serial_number" defaultValue={assignment?.serial_number ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm" placeholder="Optional" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Assigned Date</label>
          <input name="assigned_at" type="date" required defaultValue={assignment?.assigned_at ?? new Date().toISOString().split('T')[0]} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Condition</label>
          <input name="condition" defaultValue={assignment?.condition ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm" placeholder="e.g. Good, New" />
        </div>
      </div>
      <div>
        <label className="block text-xs text-zinc-500 dark:text-gray-400 mb-1">Notes</label>
        <textarea name="notes" rows={2} defaultValue={assignment?.notes ?? ''} className="w-full rounded bg-zinc-200 dark:bg-gray-900 border border-zinc-300 dark:border-gray-700 px-3 py-2 text-zinc-900 dark:text-white text-sm" />
      </div>
      {error && <p className="text-red-400 text-sm">{error}</p>}
      <div className="flex gap-2">
        <button type="submit" disabled={loading} className="px-3 py-1.5 rounded bg-[#C27E00] text-white text-sm disabled:opacity-50">
