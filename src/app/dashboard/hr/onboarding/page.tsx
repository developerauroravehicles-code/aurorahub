...
export default async function OnboardingPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-semibold text-white mb-2">Onboarding</h1>
        <p className="text-gray-400">Manage onboarding checklists for platform employees.</p>
      </div>

      <OnboardingTaskForm employees={employees || []} />

      <div>
        <h2 className="text-lg font-semibold text-white mb-4">Onboarding Tasks by Employee</h2>
        <div className="space-y-6">
          {Object.entries(tasksByEmployee).map(([profileId, empTasks]) => (
            <div
              key={profileId}
              className="bg-white/5 rounded-lg border border-gray-800 p-4"
            >
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium border ${
                          STATUS_COLORS[t.status] ?? 'bg-gray-800 text-gray-300'
                        }`}
                      >
                        {t.status.replace('_', ' ')}
                      </span>
        ...
        {Object.keys(tasksByEmployee).length === 0 && (
          <div className="p-8 text-center text-gray-400 bg-white/5 rounded-lg border border-gray-800">
