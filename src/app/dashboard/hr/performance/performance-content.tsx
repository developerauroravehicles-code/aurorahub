  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          ...
          <div className="mt-4 overflow-x-auto">
              ...
              <tbody className="divide-y divide-gray-800">
                {metrics.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td className="px-4 py-2 text-gray-300">{METRIC_TYPES[m.metric_type ?? ''] ?? m.metric_type ?? '—'}</td>
                    <td className="px-4 py-2 text-gray-300">{m.value != null ? Number(m.value).toFixed(2) : '—'}</td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <button onClick={() => { setEditingMetricId(m.id); setShowMetricForm(true) }} className="p-1.5 text-gray-400 hover:text-[#C27E00] mr-1" title="Edit">
            ...
            {metrics.length === 0 && !showMetricForm && (
              <p className="text-gray-500 py-6 text-center">No metrics yet. Add success rates, quality scores, etc.</p>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'feedback' && (
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-white">Performance Feedback</h2>
            <button
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-800 text-sm">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-gray-400">Rating</th>
                  <th className="px-4 py-2 text-left text-gray-400">Comment</th>
                  <th className="px-4 py-2 text-right text-gray-400">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800">
                {feedback.map((f) => (
                  <tr key={f.id}>
                    ...
                    <td className="px-4 py-2 text-gray-300">{FEEDBACK_TYPES[f.feedback_type ?? ''] ?? f.feedback_type ?? '—'}</td>
                    <td className="px-4 py-2 text-gray-400">{f.source ?? '—'}</td>
                    <td className="px-4 py-2 text-gray-300">{f.rating != null ? Number(f.rating).toFixed(1) : '—'}</td>
                    <td className="px-4 py-2 text-gray-400 max-w-[200px] truncate">{f.comment ?? '—'}</td>
                    <td className="px-4 py-2 text-right">
                      <button onClick={() => { setEditingFeedbackId(f.id); setShowFeedbackForm(true) }} className="p-1.5 text-gray-400 hover:text-[#C27E00] mr-1" title="Edit">
                        <Pencil className="w-4 h-4 inline" />
                      </button>
            ...
            {feedback.length === 0 && !showFeedbackForm && (
              <p className="text-gray-500 py-6 text-center">No feedback yet. Add customer, manager, or quality feedback.</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="bg-white/5 rounded-lg border border-gray-800 p-6">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-800 text-sm">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-gray-400">Reviewer</th>
                  <th className="px-4 py-2 text-left text-gray-400">Rating</th>
              ...
              <tbody className="divide-y divide-gray-800">
                {reviews.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="px-4 py-2 text-gray-300">{new Date(r.review_date).toLocaleDateString()}</td>
                    <td className="px-4 py-2 text-gray-400">{r.reviewer?.full_name ?? '—'}</td>
                    <td className="px-4 py-2 text-gray-300">{r.rating != null ? Number(r.rating).toFixed(1) : '—'}</td>
                    <td className="px-4 py-2">
                      <span className={`px-2 py-0.5 rounded text-xs ${
                        r.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                        r.status === 'scheduled' ? 'bg-blue-500/20 text-blue-400' :
                        r.status === 'cancelled' ? 'bg-gray-600 text-gray-400' :
                    ...
                    <td className="px-4 py-2 text-right">
                      <button onClick={() => { setEditingReviewId(r.id); setShowReviewForm(true) }} className="p-1.5 text-gray-400 hover:text-[#C27E00] mr-1" title="Edit">
                        <Pencil className="w-4 h-4 inline" />
                      </button>
            ...
            {reviews.length === 0 && !showReviewForm && (
              <p className="text-gray-500 py-6 text-center">No reviews yet. Schedule periodic performance reviews.</p>
            )}
          </div>
        </div>
      )}
...
function MetricForm({ personnel, metric, onClose, onSuccess }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 p-4 rounded bg-black/30 border border-gray-700 space-y-3">
      <h3 className="text-white font-medium">{isEdit ? 'Edit Metric' : 'Add Metric'}</h3>
      {!isEdit && (
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <select name="metric_type" defaultValue={metric?.metric_type ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
            <option value="">—</option>
        ...
        <div>
          ...
          <input name="value" type="number" step="0.01" defaultValue={metric?.value != null ? String(metric.value) : ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" placeholder="e.g. 95.5" />
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Period Start</label>
          <input name="period_start" type="date" defaultValue={metric?.period_start ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Period End</label>
          <input name="period_end" type="date" defaultValue={metric?.period_end ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
        </div>
      </div>
...
function FeedbackForm({ personnel, item, onClose, onSuccess }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 p-4 rounded bg-black/30 border border-gray-700 space-y-3">
      <h3 className="text-white font-medium">{isEdit ? 'Edit Feedback' : 'Add Feedback'}</h3>
      {!isEdit && (
        <div>
          <label className="block text-xs text-gray-400 mb-1">Personnel</label>
          <select name="personnel_id" required className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-gray-400 mb-1">Feedback Type</label>
          <select name="feedback_type" defaultValue={item?.feedback_type ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
            <option value="">—</option>
        ...
        <div>
          ...
          <input name="source" defaultValue={item?.source ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" placeholder="e.g. Job #1234" />
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Rating (1-5)</label>
          <input name="rating" type="number" step="0.1" min="1" max="5" defaultValue={item?.rating != null ? String(item.rating) : ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
        </div>
      </div>
      <div>
        <label className="block text-xs text-gray-400 mb-1">Comment</label>
        <textarea name="comment" rows={2} defaultValue={item?.comment ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
      </div>
      {error && <p className="text-red-400 text-sm">{error}</p>}
      <div className="flex gap-2">
        <button type="submit" disabled={loading} className="px-3 py-1.5 rounded bg-[#C27E00] text-white text-sm disabled:opacity-50">
...
function ReviewForm({ personnel, review, onClose, onSuccess }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 p-4 rounded bg-black/30 border border-gray-700 space-y-3">
      <h3 className="text-white font-medium">{isEdit ? 'Edit Review' : 'Add Review'}</h3>
      {!isEdit && (
        <div>
          <label className="block text-xs text-gray-400 mb-1">Personnel</label>
          <select name="personnel_id" required className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <input name="review_date" type="date" required defaultValue={review?.review_date ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Reviewer</label>
          <select name="reviewer_id" defaultValue={review?.reviewer_id ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
            <option value="">—</option>
            {personnel.map((p) => <option key={p.id} value={p.id}>{p.full_name}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Rating (1-5)</label>
          <input name="rating" type="number" step="0.1" min="1" max="5" defaultValue={review?.rating != null ? String(review.rating) : ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Status</label>
          <select name="status" defaultValue={review?.status ?? 'draft'} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm">
      ...
      <div>
        ...
        <textarea name="notes" rows={3} defaultValue={review?.notes ?? ''} className="w-full rounded bg-gray-900 border border-gray-700 px-3 py-2 text-white text-sm" />
      </div>
      {error && <p className="text-red-400 text-sm">{error}</p>}
      <div className="flex gap-2">
        <button type="submit" disabled={loading} className="px-3 py-1.5 rounded bg-[#C27E00] text-white text-sm disabled:opacity-50">
