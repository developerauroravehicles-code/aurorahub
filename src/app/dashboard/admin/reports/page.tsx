  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-2">Admin Reports</h1>
          <p className="text-zinc-500 dark:text-gray-400">View detailed reports filtered by dealer and specialist</p>
        </div>
      ...
      <div className="bg-zinc-200/50 dark:bg-white/5 border border-zinc-200 dark:border-gray-800 p-6 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Dealer Filter */}
          <div>
            <label className="block text-sm font-medium text-zinc-600 dark:text-gray-300 mb-2">Dealer</label>
            <select
              value={selectedDealerId}
              onChange={(e) => setSelectedDealerId(e.target.value)}
              disabled={userRole === 'general_manager'}
              className="w-full border border-zinc-300 dark:border-gray-700 bg-zinc-200/50 dark:bg-white/5 p-2 rounded text-white focus:outline-none focus:ring-1 focus:ring-[#C27E00] focus:border-[#C27E00] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {userRole === 'general_manager' ? (
                dealers.map(dealer => (
                  <option key={dealer.id} value={dealer.id} className="bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white">
                    {dealer.name}
                  </option>
                ))
              ) : (
                <>
                  <option value="all" className="bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white">All Dealers</option>
                  {dealers.map(dealer => (
                    <option key={dealer.id} value={dealer.id} className="bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white">
                      {dealer.name}
                    </option>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-600 dark:text-gray-300 mb-2">Personel</label>
            <select
              ...
              className="w-full border border-zinc-300 dark:border-gray-700 bg-zinc-200/50 dark:bg-white/5 p-2 rounded text-white focus:outline-none focus:ring-1 focus:ring-[#C27E00] focus:border-[#C27E00] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="all" className="bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white">All Personnel</option>
              {Object.entries(employeesByRole).map(([role, roleEmployees]) => (
                <optgroup key={role} label={roleLabels[role] || role} className="bg-zinc-50 dark:bg-black">
                  {roleEmployees.map(employee => (
                    <option key={employee.id} value={employee.id} className="bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white pl-4">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-zinc-200/50 dark:bg-white/5 border border-zinc-200 dark:border-gray-800 p-6 rounded-lg">
              <h3 className="text-sm font-medium text-zinc-500 dark:text-gray-400 mb-2">Total Demands</h3>
                  ...
                  .map(([camera, count]) => (
                    <div key={camera} className="flex items-center justify-between p-3 bg-zinc-200/50 dark:bg-white/5 rounded border border-zinc-200 dark:border-gray-800">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-zinc-500 dark:text-gray-400">{count} demand{count !== 1 ? 's' : ''}</span>
          ...
          {/* Detailed List */}
          <div className="bg-zinc-200/50 dark:bg-white/5 border border-zinc-200 dark:border-gray-800 p-6 rounded-lg">
            <h2 className="text-lg font-semibold text-zinc-900 dark:text-white mb-4">Detailed Demand List</h2>
            {demands.length === 0 ? (
              <p className="text-zinc-500 dark:text-gray-400 text-center py-8">No demands found for the selected filters.</p>
            ) : (
