  ...
  return (
    <div className="bg-white/5 border border-gray-800 rounded-lg p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-white">Demand Overview</h2>
        <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="min-h-[200px]">
          {total === 0 ? (
            <p className="text-gray-500 text-sm py-8">No demands yet.</p>
          ) : data.length === 0 ? (
            <p className="text-gray-500 text-sm py-8">No data to display.</p>
          ) : (
        ...
        <div>
          <h3 className="text-sm font-medium text-gray-400 mb-2">Recent Demands</h3>
          {recentDemands.length === 0 ? (
            <p className="text-gray-500 text-sm">No recent demands.</p>
          ) : (
                ...
                return (
                  <li key={d.id}>
                    <Link
                      ...
                      className="flex items-center gap-2 p-2 rounded-lg border border-gray-700 bg-black/20 hover:bg-black/30 transition-colors"
                    >
                      <span className="text-sm font-medium text-white truncate flex-1">
                        {d.customer_firstname} {d.customer_lastname}
                        {d.demand_number != null && (
                          <span className="text-gray-500 text-xs ml-1">#{d.demand_number}</span>
                        )}
                      </span>
                      <span className={`text-xs px-2 py-0.5 rounded ${statusColors[d.status] ?? 'bg-gray-800 text-gray-400'}`}>
                        {d.status.replace('_', ' ')}
                      </span>
                    </Link>
                  </li>
