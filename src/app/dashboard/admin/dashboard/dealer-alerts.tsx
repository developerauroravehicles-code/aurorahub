  ...
  if (loading) {
    return (
      <div className="bg-white/5 border border-gray-800 rounded-lg p-6">
        <h2 className="text-lg font-semibold text-white mb-4">Dealer Alerts</h2>
          ...
          {alerts.slice(0, 20).map((alert, i) => {
            ...
            return (
              <li key={`${alert.dealerId}-${alert.type}-${i}`}>
                <Link
                  href={config.href}
                  className="flex items-center gap-3 p-3 rounded-lg border border-gray-700 bg-black/20 hover:bg-black/30 transition-colors"
                >
                  <Icon className={`w-4 h-4 shrink-0 ${config.color}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{alert.dealerName}</p>
                    <p className="text-xs text-gray-500">{config.label}</p>
                  </div>
                  {alert.count > 1 && (
                    <span className="text-xs font-medium text-gray-400">{alert.count}</span>
                  )}
                </Link>
              </li>
            )
