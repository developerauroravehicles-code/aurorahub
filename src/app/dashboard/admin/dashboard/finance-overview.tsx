...
export function FinanceOverview({ summary, selectedMonth = '' }: { summary: FinanceSummary; selectedMonth?: string }) {
  ...
  return (
    <div className="bg-zinc-200/50 dark:bg-zinc-200/50 dark:bg-white/5 border border-zinc-200 dark:border-zinc-200 dark:border-gray-800 rounded-lg p-6">
      ...
      {invoiceCount === 0 ? (
        <p className="text-zinc-500 dark:text-zinc-500 dark:text-gray-500 text-sm">No invoice data yet. Complete demands and add invoice amounts.</p>
      ) : (
        <>
          {/* Summary Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div className="p-3 rounded-lg border border-zinc-300 dark:border-zinc-300 dark:border-gray-700 bg-zinc-50 dark:bg-black/20">
              <p className="text-xs text-zinc-500 dark:text-zinc-500 dark:text-gray-500 mb-1">Total Invoiced</p>
              <p className="text-xl font-bold text-[#C27E00]">{fmt(totalInvoiced)}</p>
            </div>
            <div className="p-3 rounded-lg border border-zinc-300 dark:border-zinc-300 dark:border-gray-700 bg-zinc-50 dark:bg-black/20">
              <p className="text-xs text-zinc-500 dark:text-zinc-500 dark:text-gray-500 mb-1">Subtotal</p>
              <p className="text-lg font-bold text-zinc-900 dark:text-zinc-900 dark:text-white">{fmt(totalSubtotal)}</p>
            </div>
            <div className="p-3 rounded-lg border border-zinc-300 dark:border-zinc-300 dark:border-gray-700 bg-zinc-50 dark:bg-black/20">
              <p className="text-xs text-zinc-500 dark:text-zinc-500 dark:text-gray-500 mb-1">Tax (GST/PST)</p>
              <p className="text-lg font-bold text-blue-400">{fmt(totalTax)}</p>
            </div>
            <div className="p-3 rounded-lg border border-zinc-300 dark:border-zinc-300 dark:border-gray-700 bg-zinc-50 dark:bg-black/20">
              <p className="text-xs text-zinc-500 dark:text-zinc-500 dark:text-gray-500 mb-1">Invoice Count</p>
          ...
          {chartData.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-zinc-500 dark:text-zinc-500 dark:text-gray-400 mb-3 flex items-center gap-1">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
                  ...
                  />
                  <Legend wrapperStyle={{ fontSize: '11px' }} formatter={(v) => <span className="text-zinc-600 dark:text-zinc-600 dark:text-gray-300">{v}</span>} />
          ...
          {byDealer.length > 0 && (
            <div className="mt-4 border-t border-zinc-300 dark:border-zinc-300 dark:border-gray-700 pt-3">
              <p className="text-xs text-zinc-500 dark:text-zinc-500 dark:text-gray-500 mb-2">Dealer breakdown</p>
              <div className="max-h-[120px] overflow-y-auto space-y-1">
                {byDealer.slice(0, 6).map(d => (
                  <div key={d.dealerName} className="flex justify-between items-center text-sm py-1 border-b border-zinc-200 dark:border-zinc-200 dark:border-gray-800 last:border-0">
                    <span className="text-zinc-600 dark:text-zinc-600 dark:text-gray-300 truncate flex-1">{d.dealerName}</span>
