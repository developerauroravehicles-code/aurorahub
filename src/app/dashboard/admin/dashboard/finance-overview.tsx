...
export function FinanceOverview({ summary }: { summary: FinanceSummary }) {
  ...
  return (
    <div className="bg-white/5 border border-gray-800 rounded-lg p-6">
      ...
      {invoiceCount === 0 ? (
        <p className="text-gray-500 text-sm">No invoice data yet. Complete demands and add invoice amounts.</p>
      ) : (
        <>
          {/* Summary Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <div className="p-3 rounded-lg border border-gray-700 bg-black/20">
              ...
              <p className="text-lg font-bold text-white">{fmt(totalSubtotal)}</p>
            </div>
            <div className="p-3 rounded-lg border border-gray-700 bg-black/20">
              <p className="text-xs text-gray-500 mb-1">Tax (GST/PST)</p>
              <p className="text-lg font-bold text-blue-400">{fmt(totalTax)}</p>
            </div>
            <div className="p-3 rounded-lg border border-gray-700 bg-black/20">
              <p className="text-xs text-gray-500 mb-1">Invoice Count</p>
          ...
          {byDealer.length > 0 && (
            <div className="mt-4 border-t border-gray-700 pt-3">
              <p className="text-xs text-gray-500 mb-2">Dealer breakdown</p>
              <div className="max-h-[120px] overflow-y-auto space-y-1">
                {byDealer.slice(0, 6).map(d => (
                  <div key={d.dealerName} className="flex justify-between items-center text-sm py-1 border-b border-gray-800 last:border-0">
                    <span className="text-gray-300 truncate flex-1">{d.dealerName}</span>
