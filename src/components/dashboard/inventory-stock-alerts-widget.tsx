    ...
    <div
      ...
      className={`rounded-xl border p-6 shadow-lg ${
        hasRisk
          ...
          : 'border-gray-800/80 bg-gradient-to-b from-white/[0.04] to-transparent'
      }`}
    >
      <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          <div
            className={`flex h-11 w-11 items-center justify-center rounded-lg ${hasRisk ? 'bg-amber-500/15' : 'bg-white/5'}`}
          >
            <Package className={`h-5 w-5 ${hasRisk ? 'text-amber-400' : 'text-[#C27E00]'}`} />
          </div>
          <div>
            <h2 className="text-lg font-semibold text-white">Inventory &amp; stock alerts</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <div className="rounded-lg border border-gray-800 bg-white/5 px-3 py-2">
          <p className="text-[10px] uppercase tracking-wider text-gray-500">Negative rows</p>
          <p className={`text-xl font-bold tabular-nums ${summary.shortSkuLines > 0 ? 'text-red-400' : 'text-white'}`}>
            {summary.shortSkuLines}
          </p>
        </div>
        <div className="rounded-lg border border-gray-800 bg-white/5 px-3 py-2">
          <p className="text-[10px] uppercase tracking-wider text-gray-500">Below min.</p>
          <p className={`text-xl font-bold tabular-nums ${summary.belowMinLines > 0 ? 'text-amber-300' : 'text-white'}`}>
            {summary.belowMinLines}
          </p>
        </div>
        <div className="rounded-lg border border-gray-800 bg-white/5 px-3 py-2">
          <p className="text-[10px] uppercase tracking-wider text-gray-500">Negative models</p>
          <p className={`text-xl font-bold tabular-nums ${summary.negModelRollups > 0 ? 'text-red-300' : 'text-white'}`}>
            {summary.negModelRollups}
          </p>
        </div>
        <div className="rounded-lg border border-gray-800 bg-white/5 px-3 py-2">
          <p className="text-[10px] uppercase tracking-wider text-gray-500">Total dealer stock</p>
          <p
            className={`text-xl font-bold tabular-nums ${summary.dealerQtyTotal < 0 ? 'text-red-400' : 'text-gray-200'}`}
          >
      ...
      {top.length === 0 ? (
        <p className="text-sm text-gray-500">No stock data or no alerts generated.</p>
      ) : (
            ...
            <li
              ...
              className={`flex gap-2 rounded-lg border px-3 py-2.5 text-sm ${
                a.level === 'warning'
                  ...
                  : 'border-gray-700/60 bg-white/[0.03] text-gray-300'
              }`}
            >
              {a.level === 'warning' && (
